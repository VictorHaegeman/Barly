...
router.get('/', auth, async (req, res, next) => {
  try {
    ...
    res.json(memStore.events);
  } catch (e) {
    next(e);
  }
});

router.post('/', auth, validate(schemas.createEvent), async (req, res, next) => {
  try {
    ...
    const ne = { id: `e${memStore.events.length + 1}`, barId, title, date: new Date(date), participants: [] };
    memStore.events.push(ne);
...
router.post('/:id/join', auth, async (req, res, next) => {
  try {
    ...
    const ev = memStore.events.find((e) => e.id === req.params.id);
    if (!ev) return res.status(404).json({ message: 'Event introuvable' });
